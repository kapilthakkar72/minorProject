...
def generateQueryForRetail(commodity,year,state):
	return "Select r.date,r.price,r.unitofprice from retailpricedata as r,commodity as co,variety as v, center as c, states as s where s.state ='"+state+"' and s.statecode = c.statecode and c.centercode = r.centercode and co.commodityname = '" + commodity + "' and co.commoditycode = v.commoditycode and v.commqualitycode = r.commqualitycode and r.date>='"+ year + "-01-01' and r.date<='" + year + "-12-31'"
...
def main():
	...
	for record in recordsW:
		...
		minimumpriceList.append(record[3]/100) # Divided by 100 to get per Kg price
		maximumpriceList.append(record[4]/100) # Divided by 100 to get per Kg price
		arrivalList.append(record[1])
		modalpriceList.append(record[5]/100) # Divided by 100 to get per Kg price
